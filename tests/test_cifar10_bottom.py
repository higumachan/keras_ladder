...
def main():
    ...
    model, output_shapes = create_noised_model(0.05)
    print model.summary()
    print model.outputs
    print output_shapes

    # lambdas of denoising costs
    lams = [
        10.0,
        0.0,
        ] + [0.0 if isinstance(layer, keras.layers.MaxPooling2D) else 0.0 for layer in layers[2:]] + [0.0]
    labeled_count = 4000
    ...
    model.fit_generator(
        ...
        callbacks=[
            ...
            PrintOutputValAccOnly(filepath="./results/ladderCNN_small/v3_model_log.txt"),
            ],
        )

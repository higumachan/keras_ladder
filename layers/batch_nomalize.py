...
class OnlyBatchNormalization(Layer):
    '''Normalize the activations of the previous layer at each batch,
    i.e. applies a transformation that maintains the mean activation
    close to 0 and the activation standard deviation close to 1.
    # Arguments
        ex.shape[:-1]psilon: small float > 0. Fuzz parameter.
    ...
    '''
    def __init__(self, epsilon=1e-5, mode=0, axis=-1, momentum=0.99, with_mean_std=False, **kwargs):
        self.epsilon = epsilon
        self.supports_masking = True
        self.mode = mode
        self.axis = axis
        self.momentum = momentum
        self.with_mean_std = with_mean_std
        if self.mode == 0:
            self.uses_learning_phase = True
        super(OnlyBatchNormalization, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.gamma = K.ones(shape, name='{}_gamma'.format(self.name))
    ...
    def call(self, x, mask=None):
        ...
        if self.with_mean_std:
            return K.concatenate(
                [
                    K.reshape(x_normed, (-1, 1) + input_shape[1:]),
                    K.reshape(mean, (-1, 1,) + input_shape[1:]),
                    K.reshape(std, (-1, 1,) + input_shape[1:])
                ],
                axis=1
            )
            return K.concatenate([x_normed, mean, std], axis=1)
        return x_normed
    ...
    def get_config(self):
        ...
        base_config = super(OnlyBatchNormalization, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

    def get_output_shape_for(self, input_shape):
        if self.with_mean_std:
            return (input_shape[0], input_shape[1] * 3) + input_shape[2:]
        return input_shape
...
class ScaleAndShift(Layer):
    ...
    # References
        - [Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift](http://jmlr.org/proceedings/papers/v37/ioffe15.html)
    '''
    def __init__(self, mode=0, axis=-1, momentum=0.99,
    ...
    def get_config(self):
        ...
        base_config = super(ScaleAndShift, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

Std = Lambda(lambda x: K.sqrt(K.var(x, 0, keepdims=True) + 1e-5))
Mean = Lambda(lambda x: K.mean(x, 0, keepdims=True))
